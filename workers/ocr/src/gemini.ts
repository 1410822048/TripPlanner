...
function buildPrompt(currencyHint?: string): string {
  return [
    ...
    '- Japan/Taiwan tax vocabulary priority: decide by whether the line changes the paid total. 内税 / 內稅 / 税込 / 含稅 / 軽税 / 軽減税率 usually identify included tax or a reduced-rate category, so put them in ignoredLines[] unless the line is explicitly added to a pre-tax subtotal. 免税 / tax-free / tax exempt becomes TAX_EXEMPT only when it reduces the paid total; otherwise put the visible line in ignoredLines[].',
