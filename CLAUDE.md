...
- **claim 必須在 `set()` 之前用 `get()` 比對**。zustand persist 沒有 dirty check(`middleware.mjs`:`set(...args); return setItem()`)—— 即使 reducer 回傳同一個 state 讓 vanilla 跳過 listener,persist 仍會寫 localStorage。而 reconcile 掛在每一次 auth 解析上,所以 `set(s => s)` 等於每次都寫一次、也每次都給一次拋錯機會(註:`onAuthStateChanged` 只在登入 / 登出 / 使用者變更時 fire,token refresh 是 `onIdTokenChanged`)
...
This project is indexed by GitNexus as **TripPlanner** (6164 symbols, 14959 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
