...
import { getPerfMarks, getPerfMetrics, isPerfDebugEnabled, subscribePerf } from '@/utils/perf'
...
export default function PerfStrip() {
  ...
  // initial render only.
  const [enabled] = useState(isPerfDebugEnabled)
  ...
  return (
    ...
    >
      {metrics.map(metric => (
        <div key={metric.name} className="mb-1 border-b border-white/20 pb-1">
          <div className="flex justify-between gap-3 font-semibold">
            <span>{metric.name}</span>
            <span className="tabular-nums">
              {metric.name === 'LCP' ? `${Math.round(metric.value)}ms` : metric.value.toFixed(3)}
            </span>
          </div>
          {metric.detail && <div className="truncate opacity-70">{metric.detail}</div>}
        </div>
      ))}
      {marks.map((m, i) => {
