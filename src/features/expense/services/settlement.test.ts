...
// ─── Cross-check fixtures with Worker settlement-domain.ts ────────
//
// MIRROR of `workers/ocr/test/settlement-domain.spec.ts` PAIRWISE_FIXTURES.
// The 8 fixtures below MUST stay in lockstep with the Worker side -- any
// drift in the pairwise algorithm fails one of these (or its sibling on
// the Worker), surfacing the mismatch BEFORE settlements get rejected /
// accepted incorrectly at the Worker create-gate.
//
// Why duplicated rather than imported: see `settlement-domain.ts` module
// docstring — the Worker can't import from src/ without dragging the
// TanStack / Firebase fans that won't bundle on V8 isolates. The cost is
// keeping two copies in sync; the benefit is two independent
// implementations that catch drift on opposite sides.
//
// Each fixture is a literal object so the file-to-file diff makes any
// semantic divergence obvious. `mkXxxLiteral` helpers below adapt the
// literals into the rich client `Expense` / `SettlementRecord` shapes.

interface CrossCheckExpense  { paidBy: string; amount: number; splits: Array<{ memberId: string; amount: number }> }
interface CrossCheckSettlement { fromUid: string; toUid: string; amount: number; createdAtMs: number }
interface CrossCheckFixture {
  name:        string
  expenses:    CrossCheckExpense[]
  settlements: CrossCheckSettlement[]
  expected:    Record<string, Record<string, number>>
}

/** Settlement with ms-explicit createdAt so cross-check fixtures stay
 *  ordering-deterministic without coupling to MOCK_TIMESTAMP's epoch-0. */
function settlementWithMs(s: CrossCheckSettlement, idx: number): SettlementRecord {
  const ms = s.createdAtMs
  const ts: import('firebase/firestore').Timestamp = {
    toDate:   () => new Date(ms),
    toMillis: () => ms,
    seconds:     Math.floor(ms / 1000),
    nanoseconds: (ms % 1000) * 1_000_000,
    isEqual:  (o: import('firebase/firestore').Timestamp) => o?.toMillis?.() === ms,
...
function expenseFromLiteral(e: CrossCheckExpense, idx: number): Expense {
  return {
    id:        `xfix_e_${idx}`,
    tripId:    'demo',
    title:     't',
...
/** Cover any uid mentioned in the fixtures so `computeBalancesFull` doesn't
 *  treat them as ghosts. Ghost-vs-known doesn't affect `pairwise` (the
 *  normalize step is purely id-keyed), but parity with the Worker side --
 *  which doesn't have the "ghost" concept -- keeps reasoning simple. */
const XFIX_MEMBERS: TripMember[] = [
  { id: 'A', label: 'A', color: '#000', bg: '#fff' },
  { id: 'B', label: 'B', color: '#000', bg: '#fff' },
  { id: 'C', label: 'C', color: '#000', bg: '#fff' },
]

const CROSS_CHECK_FIXTURES: CrossCheckFixture[] = [
  {
...
describe('computeBalancesFull cross-check fixtures (Worker mirror)', () => {
  for (const fx of CROSS_CHECK_FIXTURES) {
    it(fx.name, () => {
      const expenses    = fx.expenses.map(expenseFromLiteral)
      const settlements = fx.settlements.map(settlementWithMs)
      const { pairwise } = computeBalancesFull(expenses, XFIX_MEMBERS, settlements)
      expect(pairwise).toEqual(fx.expected)
    })
  }
})
