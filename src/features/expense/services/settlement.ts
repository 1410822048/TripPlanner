...
// that's a UI concern). Step 5 + paid/owed display + ghost rows are
// UI-only and stay client-side; the chronological orphan-reason replay
// that labels each orphan's `reason` lives in the sibling
// ./settlementOrphanReason module.
//
...
import {
  buildOrphanReasonMap,
  classifyOrphan,
  type OrphanReason,
} from './settlementOrphanReason'
...
// `OrphanReason` + the chronological-replay internals moved to
// ./settlementOrphanReason. Re-exported here so existing consumers
// (SettlementSummary / SettlementHistory / settlementOrphanCopy) keep
// importing it from '../services/settlement' unchanged.
 ...
 *  fallback-assign idiom across the pairwise maps (gross/applied).
 *  `settlementOrphanReason.ts` keeps its own 4-line copy so the replay
 *  file stays standalone (no cross-import for a generic helper). */
