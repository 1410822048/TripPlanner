...
// `selectedTripId` (persisted to localStorage) so the query can fire
// on cold load — collapses the gap between "header rendered" and
// "list arrived" on /bookings hard reload. TanStack Query dedupes
// against BookingsPage's own useBookings call, so the list resolves
