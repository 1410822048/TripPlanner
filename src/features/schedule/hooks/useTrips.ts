// src/features/schedule/hooks/useTrips.ts
...
import { createTrip, deleteTrip, getMyTrips, updateTrip } from '../services/tripService'
...
export function useCreateTrip() {
  ...
  return useMutation({
    ...
    onSuccess: (trip, { user }) => {
      // Seed the list cache with the new trip so the switcher picks it up
      // without a round-trip; useMyTrips will still invalidate on focus.
...
export function useDeleteTrip(uid: string | undefined) {
  ...
  return useMutation({
    ...
    onMutate: (tripId) => {
      ...
      const key  = tripKeys.mine(uid)
      const prev = qc.getQueryData<Trip[]>(key)
      if (prev) qc.setQueryData<Trip[]>(key, prev.filter(t => t.id !== tripId))
      return { prev }
    },
    onError: (err, _vars, ctx) => {
      if (uid && ctx?.prev !== undefined) qc.setQueryData(tripKeys.mine(uid), ctx.prev)
    ...
    onSettled: () => {
      if (uid) qc.invalidateQueries({ queryKey: tripKeys.mine(uid) })
    },
  })
}
